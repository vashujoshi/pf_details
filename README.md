...
3. **Execution**:
   - To execute this management command, run:
     ```bash
...
1. **By Running the fetchpfdata Management Command**: This is done via the command line using:
   ```bash
   python <fetchpfdata>.py
   ```
   This command processes the Excel and XML files and stores the data in the `Company`, `Employee`, and `PFPayment` models.
...
To manually fetch and populate the database with PF data, visit `http://localhost:8000/fetch_pf/` or run the management command:

```bash
python commandname.py
```

---

### **7. Running Management Command in the Script**
To run the management command programmatically within the script, include the following:
...
if __name__ == "__main__":
    app._prepare()
    command = fetchpfdata()
    command.handle(arg1='value')  # Adjust 'arg1' and 'value' as needed
```
